import React from "react";

const LastSection = () => {
  return (
    <section className="bg-white max-w-7xl mx-auto pt-8">
      <p className="text-center p-2.5 text-lg font-medium">
        We are a digital platform that gives you the opportunity to submit
        honest reviews for products that we provide you with free samples of. We
        are not the manufacturers or suppliers of these products and assume no
        liability in relation to the products or their use.
      </p>
    </section>
  );
};

export default LastSection;
